...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-[#faf9f7] font-sans text-stone-900">
      ...
      {/* Helper dev tools to quickly navigate the prototype */}
      <div className="fixed bottom-4 right-4 bg-black/80 text-white text-[11px] p-2 rounded flex gap-2 z-50 shadow-xl backdrop-blur-sm">
        <span className="opacity-50">Dev Nav:</span>
        <button onClick={() => setCurrentView("dashboard")} className="hover:text-emerald-400">Dash</button>
        <button onClick={() => setCurrentView("join")} className="hover:text-emerald-400">Join</button>
        <button onClick={() => setCurrentView("pre-call")} className="hover:text-emerald-400">Pre-call</button>
        <button onClick={() => setCurrentView("waiting-guest")} className="hover:text-emerald-400">Guest</button>
        <button onClick={() => setCurrentView("waiting-host")} className="hover:text-emerald-400">Host</button>
      </div>
    </div>
  );
}
